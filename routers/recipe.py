...
async def get_all_recipes(db: db_dependency):
    return (
        db.query(Recipe)
        .filter((Recipe.isDeleted == False) & (Recipe.isHide == False))
        .all()
    )
...
async def get_recipe_by_people_count(
    db: db_dependency, people_count: int, user: user_dependency
):
    if user is None:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED, detail="Unauthorize User"
    ...
    recipes = (
        db.query(Recipe)
        .filter(
            Recipe.peopleCount == people_count,
            Recipe.isDeleted == False,
            Recipe.isHide == False,
        )
        .all()
...
async def search_recipes(db: db_dependency, query: str):
    recipes = (
        db.query(Recipe)
        .filter(
            (Recipe.recipeName.like(f"%{query}%"))
...
async def get_recipes_by_like(db: db_dependency):
    ...
    recipes = (
        db.query(Recipe)
        .filter(Recipe.isDeleted == False, Recipe.isHide == False)
        .order_by(Recipe.likesCount.desc())
        .all()
    )
...
async def create_recipe(
    db: db_dependency, user: user_dependency, create_recipe: RecipeRequest
):
    if user is None:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED, detail="Unauthorize User"
...
def like_recipe(recipe_id: int, user: user_dependency, db: db_dependency):
    ...
    if not recipe:
        raise HTTPException(
            ...
            detail="Recipe not found or unavailable.",
        )

    existing_like = (
        db.query(RecipeLikes)
        .filter(
            (RecipeLikes.userId == user.get("id")) & (RecipeLikes.recipeId == recipe_id)
        )
        .first()
    )
...
async def update_recipe(
    ...
    recipe_model = (
        ...
        .filter(
            Recipe.userId == user.get("id"),
            Recipe.id == recipe_id,
            Recipe.isDeleted == False,  # Ensure recipe is not deleted
            Recipe.isHide == False,
        )
        .first()
    )
...
async def delete_recipe(db: db_dependency, user: user_dependency, recipe_id: int):
    ...
    recipe_model = (
        db.query(Recipe)
        .filter(Recipe.userId == user.get("id"), Recipe.id == recipe_id)
        .first()
    )
