...
async def get_recipe_ingredients(db: db_dependency, recipe_id: int):
    ingredients = (
        ...
        .filter(RecipeIngredients.recipeId == recipe_id)
        .all()
    )
...
async def get_ingredient_by_name(
    user: user_dependency, db: db_dependency, ingredient_name: str
):
    if user is None:
        raise HTTPException(
    ...
    ingredient_search = (
        ...
        .filter(Ingredients.ingredientName.ilike(f"%{ingredient_name}%"))
        .all()
    )
...
async def add_recipe_ingredient(
    ...
    ingredient = (
        db.query(Ingredients)
        .filter(
            Ingredients.ingredientName == ingredient_request.ingredientName.capitalize()
        )
        .first()
    )
...
@router.put(
    "/{recipe_id}/ingredients/{ingredient_id}", status_code=status.HTTP_204_NO_CONTENT
)
async def update_recipe_ingredient(
    ...
    recipe_ingredient = (
        ...
        .filter(
            (RecipeIngredients.recipeId == recipe_id)
            & (RecipeIngredients.ingredientId == ingredient_id)
        )
        .first()
    )
...
async def delete_recipe_ingredient(
    user: user_dependency, db: db_dependency, recipe_id: int, ingredient_id: int
):
    if user is None:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED, detail="Unauthorized User"
    ...
    ingredient = (
        db.query(RecipeIngredients)
        .filter_by(recipeId=recipe_id, ingredientId=ingredient_id)
        .first()
    )
