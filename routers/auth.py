...
def create_access_token(username: str, user_id: int, role: AddUserRole, expires_delta: timedelta):
...
async def create_user(db: db_dependency, create_user: CreateUserRequest):
    existing_user = db.query(Users).filter((Users.username == create_user.username) | (Users.email == create_user.email)).first()
