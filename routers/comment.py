...
async def get_all_comments(db: db_dependency, recipe_id: int):
    comments = db.query(RecipeComments).filter(
        RecipeComments.recipeId == recipe_id).all()
...
async def get_all_comments(db: db_dependency, recipe_id: int, comment_id: int):
    comment = db.query(RecipeComments).filter(
        (RecipeComments.recipeId == recipe_id) & (RecipeComments.id == comment_id)).first()
    ...
    def get_replies(parent_id):
        replies = db.query(RecipeComments).filter(
            RecipeComments.parentCommentId == parent_id).all()
...
async def add_comment(user: user_dependency, db: db_dependency, recipe_id: int, comment_request: CommentRequest):
    if user is None:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED, detail="Unauthorize User")

    recipe_commented = db.query(Recipe).filter(
    ...
    if comment_request.parentCommentId:
        parent_comment = db.query(RecipeComments).filter(
            (RecipeComments.recipeId == recipe_id) & (RecipeComments.id == comment_request.parentCommentId)).first()

        if not parent_comment:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="Parent comment does not exists.")
...
async def update_comment(user: user_dependency, db: db_dependency, recipe_id: int, comment_id: int, comment_request: CommentRequest):
    if user is None:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED, detail="Unauthorize User")

    recipe_comment = db.query(RecipeComments).filter(
        (RecipeComments.id == comment_id) & (RecipeComments.userId == user.get('id'))).first()
...
async def delete_comment(user: user_dependency, db: db_dependency, recipe_id: int, comment_id: int):
    if user is None:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED, detail="Unauthorize User")
    ...
    if comment.userId != user.get('id'):
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="You can not delete this comment.")
