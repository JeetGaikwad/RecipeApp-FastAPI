...
def follow_user(followee_id: int, user: user_dependency, db: db_dependency):
    ...
    existing_follow = db.query(Follows).filter((Follows.followerId == user.get(
        'id')) & (Follows.followeeId == followee_id)).first()

    if existing_follow:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="You already follow the user")
...
def unfollow_user(followee_id: int, user: user_dependency, db: db_dependency):
    ...
    follow = db.query(Follows).filter((Follows.followerId == follower_id) & (
        Follows.followeeId == followee_id)).first()
...
async def change_password(db: db_dependency, user: user_dependency, user_verificaiton: UserVerification):
    if user is None:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED, detail='Unauthorized User')
...
async def update_profile(db: db_dependency, user: user_dependency, update_user: UpdateUserRequest):
    if user is None:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED, detail='Unauthorized User')
