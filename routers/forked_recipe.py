...
router = APIRouter(prefix="/forked-recipe", tags=["forked-recipe"])
...
async def get_all_forked_recipes(user: user_dependency, db: db_dependency):
    ...
    recipes = (
        db.query(ForkedRecipes).filter(ForkedRecipes.userId == user.get("id")).all()
    )
...
async def get_forked_recipe_by_id(
    user: user_dependency, db: db_dependency, forked_id: int
):
    if user is None:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED, detail="Unauthorize user."
    ...
    recipe = (
        db.query(ForkedRecipes)
        .filter(
            (ForkedRecipes.userId == user.get("id")) & (ForkedRecipes.id == forked_id)
        )
        .first()
    )
...
async def update_forked_recipe(
    ...
    db: db_dependency,
    forked_id: int,
    forked_recipe_request: ForkedRecipeRequest,
):
    if user is None:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED, detail="Unauthorize user."
    ...
    existing_fork = (
        db.query(ForkedRecipes)
        .filter(
            (ForkedRecipes.id == forked_id) & (ForkedRecipes.userId == user.get("id"))
        )
        .first()
    )
...
async def delete_forked_recipe(
    user: user_dependency, db: db_dependency, forked_id: int
):
    if user is None:
        raise HTTPException(
